import ShaderLib from '../shaders/OceanShader'

export default class Ocean {
  constructor ( renderer, camera, scene, options, THREE ) {
    this.THREE = THREE
  ...
  generateMesh () {
    var geometry = new this.THREE.PlaneBufferGeometry( this.geometrySize, this.geometrySize, this.geometryResolution, this.geometryResolution );

    geometry.rotateX( - Math.PI / 2 );

    this.oceanMesh = new this.THREE.Mesh( geometry, this.materialOcean );
  ...
  generateSeedPhaseTexture() {
    ...
    this.pingPhaseTexture = new this.THREE.DataTexture( phaseArray, this.resolution, this.resolution, this.THREE.RGBAFormat );
    this.pingPhaseTexture.wrapS = this.THREE.ClampToEdgeWrapping;
    this.pingPhaseTexture.wrapT = this.THREE.ClampToEdgeWrapping;
    this.pingPhaseTexture.type = this.THREE.FloatType;
  ...
  renderWavePhase() {
    ...
    this.materialPhase.uniforms.u_deltaTime.value = this.deltaTime / 50;
  ...
  animate (camera, deltaTime) {
    let currentTime = new Date().getTime();
    this.deltaTime = ( currentTime - this.lastTime ) / 50 || 0.0;
    this.render( this.deltaTime );
    this.overrideMaterial = this.materialOcean;
    
    if ( this.changed ) {
      this.materialOcean.uniforms.u_size.value = this.size;
      this.materialOcean.uniforms.u_sunDirection.value.set(this.sunDirectionX, this.sunDirectionY, this.sunDirectionZ );
      this.materialOcean.uniforms.u_exposure.value = this.exposure;
      this.changed = false;
    }
    this.materialOcean.uniforms.u_normalMap.value = this.normalMapFramebuffer.texture;
    this.materialOcean.uniforms.u_displacementMap.value = this.displacementMapFramebuffer.texture;
    this.materialOcean.uniforms.u_projectionMatrix.value = camera.projectionMatrix;
    this.materialOcean.uniforms.u_viewMatrix.value = camera.matrixWorldInverse;
    this.materialOcean.uniforms.u_cameraPosition.value = camera.position;
